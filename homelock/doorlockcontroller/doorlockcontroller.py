...
class DoorLockController:
    def __init__(self):
        GPIO.cleanup()
        # Blue wire out of lock. Pin to set high to lock.
        self.__motor_lock_pin = 4
        # Orange wire out of lock. Pin to set high to unlock.
        self.__motor_unlock_pin = 18
        # Limit switch that signals when the lock is opened/closed.
        self.__motor_switch_pin = 27
        # Set depending on the lock direction: True if a clockwise spin locks or FALSE
        # if a counter-clockwise spin locks.
        self.__lock_direction_clockwise = True
        ...
        GPIO.add_event_detect(
            self.__motor_switch_pin, GPIO.BOTH, callback=self.status_change_callback
        )
    ...
    def status_change_callback(self, channel):
        ...
        if current_time - self.last_event_time > 0.25:
            ...
            print(
                ...
                + ": Door "
                + ("locked" if self.get() else "unlocked")
            )
            sys.stdout.flush()
    ...
    def set(self, status):
        if status != self.get():
            ...
            # Run loop until the timeout is hit or until door is at desired state,
